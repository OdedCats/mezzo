...
Busline::Busline (int id_, int opposite_id_, string name_, Busroute* busroute_, vector <Busstop*> stops_, Vtype* vtype_, ODpair* odpair_, int holding_strategy_, float ratio_headway_holding_, double init_occup_per_stop_, int nr_stops_init_occup_, int coordinated_line_id_, int coordinated_stop_id_, double planned_headway_, double desired_offset_):
	id(id_), opposite_id(opposite_id_), name(name_), busroute(busroute_), stops(stops_), vtype(vtype_), odpair(odpair_), holding_strategy(holding_strategy_), ratio_headway_holding(ratio_headway_holding_), init_occup_per_stop(init_occup_per_stop_), nr_stops_init_occup(nr_stops_init_occup_), coordinated_line_id(coordinated_line_id_), coordinated_stop_id(coordinated_stop_id_), planned_headway(planned_headway_), desired_offset(desired_offset_)
{
	active=false;
}
...
double Busline::extra_disruption_on_segment (Busstop* next_stop, double time, double time_at_stop)
{
	if (disruption_times.count(next_stop) > 0 && time > disruption_times[next_stop].second.first && time < disruption_times[next_stop].second.second) // if disruption starts from this stop and within the time window 
	{
		...
		{
			double expected_headway = time_at_stop - next_stop->get_last_departure(this);
