...
public:
	...
	Busline (int id_, int opposite_id_, string name_, Busroute* busroute_, vector <Busstop*> stops_, Vtype* vtype_, ODpair* odpair_, int holding_strategy_, float ratio_headway_holding_, double init_occup_per_stop_, int nr_stops_init_occup_); //!< Initialising constructor
	Busline (int id_, int opposite_id_, string name_, Busroute* busroute_, vector <Busstop*> stops_, Vtype* vtype_, ODpair* odpair_, int holding_strategy_, float ratio_headway_holding_, double init_occup_per_stop_, int nr_stops_init_occup_, int coordinated_line_id_, int coordinated_stop_id_, double planned_headway_, double desired_offset_);
	...
	int get_coordinated_line_id () {return coordinated_line_id;}
	int get_coordinated_stop_id () {return coordinated_stop_id;}
	double get_desired_offset () {return desired_offset;}
	double get_planned_headway () {return planned_headway;}
	...
	void set_coordinated_line (Busline* line_) {coordinated_line = line_;}
	void set_coordinated_stop (Busstop* stop_) {coordinated_stop = stop_;}
...
//	double calc_next_scheduled_arrival_at_stop (Busstop* stop, double time); // returns the remaining time till the next trip on this line is scheduled to arrive at a stop (according to schedule only) 
	...
	double extra_disruption_on_segment (Busstop* next_stop, double time, double time_at_stop);
...
protected:
	int id; //!< line ID
	int opposite_id, coordinated_line_id, coordinated_stop_id; // !< the line ID of the opposite direction and the coordinated control line and stop (relevant in case of headway control type 10)
	//Busline* opposite_line;
	Busline* coordinated_line;
	Busstop* coordinated_stop;
...
// functions for corridor control operations
	/*
	double calc_optimal_corridor_holding (Bustrip* trip);
	double calc_corridor_holding_D (Bustrip* trip);
	double calc_corridor_holding_A (Bustrip* trip);
	double calc_corridor_holding_Lambda (Bustrip* trip);
	double calc_delta_planned_offset (Bustrip* trip);
	double calc_corridor_holding_B (Bustrip* trip);
	double calc_corridor_holding_X (Bustrip* trip);
	double calc_A_dominator_holding (Bustrip* trip);
	double calc_A_nominator_holding (Bustrip* trip);
	double calc_pax_b_here_a_down (Bustrip* trip); 
	double calc_pax_b_up_a_here (Bustrip* trip);
	*/
