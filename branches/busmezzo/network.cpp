...
bool Network::readtransitnetwork(string name) //!< reads the stops, distances between stops, lines, trips and travel disruptions
{
	...
	// for lines with corridor control strategy, need to set the input coordinated line and stop
	for (vector <Busline*>::iterator line_iter = buslines.begin(); line_iter < buslines.end(); line_iter++)
	{
		if ((*line_iter)->get_holding_strategy() == 10)
		{
			Busline* coordinated_line = (*(find_if(buslines.begin(), buslines.end(), compare <Busline> ((*line_iter)->get_coordinated_line_id() )))); // find the line 
			Busstop* coordinated_stop = (*(find_if((*line_iter)->stops.begin(), (*line_iter)->stops.end(), compare <Busstop> ((*line_iter)->get_coordinated_stop_id() )))); // find the stop 
			(*line_iter)->set_coordinated_line (coordinated_line);
			(*line_iter)->set_coordinated_stop (coordinated_stop);
		}
	}
	/*
	for (vector<Busline*>::iterator line_iter = buslines.begin(); line_iter < buslines.end(); line_iter++)
...
{
  char bracket;
  int busline_id, opposite_busline_id, coordinated_line_id, coordinated_stop_id, ori_id, dest_id, route_id, vehtype, holding_strategy, nr_stops, stop_id, nr_tp, tp_id, nr_stops_init_occup;
  float ratio_headway_holding;
  double init_occup_per_stop, planned_headway, desired_offset;
  string name;
  ...
  Busstop* tp;
	...
	in >> busline_id >> opposite_busline_id >> name >> ori_id >> dest_id >> route_id >> vehtype >> holding_strategy >> ratio_headway_holding;
	if (holding_strategy == 10)
	{
		in >> coordinated_line_id >> coordinated_stop_id >> planned_headway >> desired_offset;
	}
	in >> nr_stops_init_occup >>  init_occup_per_stop >> nr_stops;
	in >> bracket;
	if (bracket != '{')
	{
	...
	if (holding_strategy == 10)
	{
		bl= new Busline (busline_id,opposite_busline_id,name,br,stops,vt,odptr,holding_strategy,ratio_headway_holding,nr_stops_init_occup,init_occup_per_stop, coordinated_line_id, coordinated_stop_id, planned_headway, desired_offset);
	}
	else
	{
		bl= new Busline (busline_id,opposite_busline_id,name,br,stops,vt,odptr,holding_strategy,ratio_headway_holding,nr_stops_init_occup,init_occup_per_stop);
	}
...
// goes over all OD pairs for a given origin to generate their path choice set
{
	...
	// second - generate indirect paths with walking distances
	cout << "Generating indirect paths:" << endl;
	vector <Busstop*>::iterator basic_origin = busstops.begin();
	for (int i=0; i < 0 ; i++)
	{
		basic_origin++;
	}
	//for (vector <Busstop*>::iterator basic_origin = busstops.begin(); basic_origin < busstops.end(); basic_origin++)
	for (basic_origin; basic_origin < busstops.end(); basic_origin++)
	{
