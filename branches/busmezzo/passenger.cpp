...
void Passenger::reset ()
{
	...
	while (new_start_time <= theParameters->start_pass_generation || new_start_time > theParameters->stop_pass_generation)
		new_start_time = start_time + random->urandom(-300, 300); //The passengers should arrive at stops a little bit randomly, otherwise they might face the exact same situation every day
